...
- `uv run pytest test/` - Run backend tests to validate the bug is fixed with zero regressions
- `cd frontend && npm run typecheck` - Run frontend type checks to validate the bug is fixed with zero regressions
