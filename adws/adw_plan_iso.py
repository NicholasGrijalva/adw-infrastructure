...
def main():
    ...
    target_branch = get_target_branch(issue)
    logger.info(f"Target branch from labels: {target_branch}")

    # Create worktree if it doesn't exist
    if not valid:
        logger.info(f"Creating worktree for {adw_id}")
        worktree_path, error = create_worktree(
            adw_id, branch_name, logger, base_branch=target_branch
        )
    ...
    plan_file_path = plan_response.output.strip().strip('`')  # Strip markdown backticks from Claude output
