...
from adw_modules.github import make_issue_comment, close_issue, ADW_BOT_IDENTIFIER
...
# These cannot be triggered directly via webhook without an ADW ID
# Non-isolated workflows (e.g., adw_plan_build) do NOT require worktrees
DEPENDENT_WORKFLOWS = [
    "adw_build_iso",
    "adw_test_iso",
    "adw_review_iso",
...
async def github_webhook(request: Request):
    ...
    try:
        ...
        # Check if this is a PR merge event - close linked issues
            ...
            if pr.get("merged"):
                ...
                print(f"PR #{pr_number} merged to {base_branch}, checking for linked issues...")

                # Parse for "Closes #X" pattern (ADW explicitly adds this)
                matches = re.findall(r'(?:closes?|fix(?:es)?|resolves?)\s+#(\d+)', pr_body, re.IGNORECASE)

                closed_issues = []
                for issue_num in matches:
                    print(f"Closing issue #{issue_num} (PR #{pr_number} merged to {base_branch})")
                    ...
                    if close_issue(issue_num):
                        ...
                        try:
                            make_issue_comment(
                                issue_num,
                                f"Closed via PR #{pr_number} merged to `{base_branch}`"
                            )
                        except Exception as e:
                            print(f"Failed to post closure comment to #{issue_num}: {e}")
