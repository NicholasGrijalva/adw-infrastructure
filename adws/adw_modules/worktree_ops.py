...
def create_worktree(
    ...
    Args:
        ...
        base_branch: The base branch to branch from (default: main)
    ...
    # Create the worktree using git, branching from origin/{base_branch}
    # Use -b to create the branch as part of worktree creation
    cmd = ["git", "worktree", "add", "-b", branch_name, worktree_path, f"origin/{base_branch}"]
...
def copy_env_file(worktree_path: str, logger: logging.Logger) -> bool:
    """Copy .env file to worktree from project root.

    Priority:
    1. .env (actual env file)
    2. .env.backup (fallback if .env doesn't exist)
    ...
    # Try .env first, then .env.backup
    source_files = [".env", ".env.backup"]

    for source_name in source_files:
        source_path = os.path.join(project_root, source_name)
        if os.path.exists(source_path):
            dest_path = os.path.join(worktree_path, ".env")
            try:
                shutil.copy2(source_path, dest_path)
                logger.info(f"Copied {source_name} to worktree as .env")
                return True
            except Exception as e:
                logger.error(f"Failed to copy {source_name}: {e}")
                return False

    logger.warning("No .env or .env.backup found in project root")
    return False


def setup_worktree_environment(worktree_path: str, backend_port: int, frontend_port: int, logger: logging.Logger) -> None:
    """Set up worktree environment by copying .env and creating .ports.env file.
    ...
    copy_env_file(worktree_path, logger)
