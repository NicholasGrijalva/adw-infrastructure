...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    """
    # Note: ANTHROPIC_API_KEY is NOT required when using Claude Code Max subscription.
    # When using Max, authentication is handled via 'claude login' OAuth tokens
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    """
    safe_env_vars = {
        # NOTE: ANTHROPIC_API_KEY is intentionally NOT passed to subprocess.
        # When using Claude Code Max subscription, authentication is handled via
        # OAuth tokens in ~/.claude/.credentials.json (from 'claude login').
        # If ANTHROPIC_API_KEY is present in the subprocess env, Claude Code will
        # prefer direct API billing over Max subscription auth.
