...
def close_issue(issue_number: str, repo: Optional[str] = None) -> bool:
    """Close a GitHub issue.

    Args:
        issue_number: Issue number to close
        repo: Optional repo path (owner/repo), uses current repo if not specified

    Returns:
        True if successful, False otherwise
    """
    # Get repo path if not provided
    if not repo:
        github_repo_url = get_repo_url()
        repo = extract_repo_path(github_repo_url)

    cmd = ["gh", "issue", "close", str(issue_number), "-R", repo]
    ...
    if result.returncode == 0:
        ...
        print(f"Error closing issue #{issue_number}: {result.stderr}")
        return False
