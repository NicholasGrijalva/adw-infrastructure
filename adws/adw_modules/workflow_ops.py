...
from adw_modules.label_ops import get_target_branch
...
# Includes both isolated (_iso) and non-isolated versions
AVAILABLE_ADW_WORKFLOWS = [
    # Non-isolated (legacy, simpler - runs in main repo)
    "adw_plan_build",

    # Isolated workflows (run in separate git worktrees)
    "adw_plan_iso",
    "adw_patch_iso",
    "adw_build_iso",
    "adw_test_iso",
...
def create_pull_request(
    ...
    if not issue:
        ...
        # NOTE: GitHubIssue already imported at module level - do NOT re-import here
        # as it causes Python scoping issues (local var shadows module import)
        try:
    ...
    if issue and isinstance(issue, GitHubIssue):
        base_branch = get_target_branch(issue)
    elif issue and isinstance(issue, dict) and "labels" in issue:
        # Handle dict case - look for target: label
        for label in issue.get("labels", []):
            label_name = label.get("name", "") if isinstance(label, dict) else str(label)
            if label_name.startswith("target:"):
                base_branch = label_name.split(":", 1)[1].strip()
                break

    request = AgentTemplateRequest(
        agent_name=AGENT_PR_CREATOR,
        slash_command="/pull_request",
        args=[branch_name, issue_json, plan_file, adw_id, base_branch],
        adw_id=adw_id,
        working_dir=working_dir,
    )
